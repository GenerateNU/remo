...
func GetOnboarded(pool *sql.DB, user_id string) (string, error) {
	...
	err := pool.QueryRow(fmt.Sprintf("SELECT onboarded FROM logins where user_id = '%s';", user_id)).Scan(&onboarded)
...
func OnboardUser(pool *sql.DB, user_id string) error {
	...
	_, err := pool.Exec(fmt.Sprintf("UPDATE logins SET onboarded='1' WHERE user_id = '%s' ;", user_id))
